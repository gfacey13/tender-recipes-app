...
export function RecipeCard({ recipe, onSwipe, style }: RecipeCardProps) {
  return (
    <motion.div
      ...
      style={style}
      className="absolute w-[90%] max-w-[400px] h-[540px] bg-white rounded-3xl shadow-2xl overflow-hidden cursor-grab active:cursor-grabbing"
      whileTap={{ scale: 0.95 }}
    >
      <div className="relative h-full">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6 text-yellow-100">
          <div className="mb-4">
          ...
          <div className="bg-gradient-to-b from-black/50 to-black/70 backdrop-blur-md p-5 rounded-2xl -mx-2 border border-white/10">
            ...
            <div className="flex flex-wrap gap-2">
              {recipe.ingredients.map((ingredient, idx) => (
                <span
                  key={idx}
                className="bg-yellow-100/30 backdrop-blur-sm px-3.5 py-1.5 rounded-full text-sm font-medium border border-yellow-200/30">
                  {ingredient}
                </span>
              ))}
            </div>
